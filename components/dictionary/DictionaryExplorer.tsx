...
function DictionaryResults({ query, onSearch }: { query: DictionaryQuery; onSearch: (query: DictionaryQuery) => void }) {
  ...
  useEffect(() => { let active = true; searchDictionary(query).then(results => { if (active) { setTerms(results); setSelected(results[0] || ""); setLoading(false); setError(null); } }).catch(problem => { if (active) { setError(problem instanceof Error && problem.name !== "AbortError" ? problem.message : "词典加载超时，请检查网络后重试。"); setLoading(false); } }); return () => { active = false; }; }, [query, revision]);
  ...
  return <div className="flex flex-col gap-6"><DictionarySearchForm initial={query} onSearch={onSearch} />
    ...
    <p className="text-xs leading-6 text-muted-foreground">基础字形 {manifest.hanzi.toLocaleString("zh-CN")} 个 · 简编本 {manifest.moeEntries.toLocaleString("zh-CN")} 条资料（含多音条目） · 开放词库 {manifest.dictionary.toLocaleString("zh-CN")} 条。<Link href="/about/#sources" className="ml-2 underline underline-offset-4">查看数据来源与范围</Link></p>
  </div>;
}
