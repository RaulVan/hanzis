...
## 2026-09-26：补齐飞花令主题、对句与每日成语

- 范围：完成 `docs/hanzi-game-research.md` 第 5 节里尚未落地的飞花令主题档、词库外输入，以及成语声韵 Wordle。人机对句按第 3.5 节降级为本地“系统给出下一句”，不接服务端或模型。听音辨字仍不做：录音书面授权未归档。
- 成功标准：既有填空飞花令可玩；主题每档至少 8 句且不挖主题字；对句对未收录句子提示“未收录”，并接出系统下一句；每日成语 6 次、四维反馈、提示、分享不含答案；lint、类型、单测、静态校验、Playwright 桌面与手机通过。
- [x] 飞花令主题与对句。
- [x] 每日成语题库、出题、反馈、提示、分享与本机连续天数。
- [x] 听音辨字：核对后仍因书面授权未归档而不实现，并写入调研状态。
- [x] 目录、sitemap、关于页、README、设计文档与全量验证。

- 飞花令新增玩法切换。主题把现有关键字收成春天、月夜、山水、风雨、秋色、冬雪，主题字留在句中。对句只接受五个或七个汉字；空输入和格式不对不记失误；含字、重复、未收录记失误。系统从当前难度再给一句。填空的「月」字入口和进度键保持原样。
- 每日成语从 chinese-xinhua 成语快照生成 `data/chengyuWordle.json`（`npm run chengyu-wordle:data`）。可猜 28,939 条四字成语，每日答案 366 条。释义 8–64 字且不包含成语本身。读音按 23 个声母拆开，y、w 写入韵母。自编接龙里的 35 条能拆读的成语优先进入答案；《童言无忌》不在该快照中，没有另补读音。`reviewStatus` 为 unreviewed。题目编号是本地日期，分享只有格子、日期和次数。
