...
| 古诗词 | `/poetry/` | 30 首校对精选（含拼音）及 11,407 篇诗词作品（自动拼音开关）、分页筛选、来源注解、收藏、朗读与选段字帖 |
| 中文字典 | `/dictionary/` | 汉字、词语、拼音、部首与笔画检索，分页结果、四来源释义与笔顺/字帖入口 |
| 汉字小游戏 | `/games/` | 六款小游戏的目录页；进度与成绩只存本机 |
| 汉字词语消除 | `/games/hanzi-match/` | 4×4 字卡按词序点选二字词，入门/进阶/挑战 15 关，原因化错误反馈、提示与星级、通关释义例句、本地进度与待复习清单 |
| 拼音快答 | `/games/pinyin-quiz/` | 看字 / 词输入拼音，不标调或标声调（数字或调号），90 / 180 秒或不限时 20 题，原因化反馈、跳过看答案、结果回看与本地最好成绩 |
| 飞花令 | `/games/feihua/` | 填空、主题限定、对句三档。对句只接受题库中的五言或七言，词库外提示“未收录”，再由系统给出下一句。题库由 `npm run feihua:data` 从诗词快照生成 |
| 古诗词排序 | `/games/poem-sort/` | 校对诗词中的五言、七言整句打乱后点选复原；入门五言、进阶七言；完成后查看拼音、译文、注释并阅读原诗 |
| 成语接龙 | `/games/chengyu-chain/` | 6 条自编四字成语链，末字接首字，四选一；接错说明首字，每环提示一次；释义与例句待人工审校 |
| 每日成语 | `/games/chengyu-wordle/` | 按本地日期出一题，6 次内猜四字成语，分字、声母、韵母、声调反馈；可看释义或一个声母。题库由 `npm run chengyu-wordle:data` 从 chinese-xinhua 成语快照生成，未经人工审校。分享只含日期和格子 |
...
诗词自动注音、增量生成和人工纠音见 [诗词拼音说明](docs/poetry-pinyin.md)。
